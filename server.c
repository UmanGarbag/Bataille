...
void* Func(void* data)
{
    ...
    while (1)
    {
        ...
        {            
            ...
            if (data.message == cred)
            {
                check_username(*sock,data.login);
                check_password(*sock,data.password);
    ...
    if(credentials != NULL){
        ...
        while(fgets(buf,SIZE_BUFFER, credentials) != NULL){
            if(strcmp(buffer,buf) == 0){
                bulldog.message = loginok;
                if(send(sock,&bulldog,paquet_size,0) != SOCKET_ERROR){
                    printf("Connecté, bienvenue %s !\n",buf);
                }
            }
        else{
            bulldog.message = loginKO;
            send(sock,&bulldog,paquet_size,0) != SOCKET_ERROR;
...
int check_password(int sock,char* buffer){
    ...
    if(password != NULL){
        while(fgets(boeuf,SIZE_BUFFER, password) != NULL){
            if(strcmp(buffer,boeuf) == 0){
                dalmatien.message = loginok;
                if(send(sock,&dalmatien,paquet_size,0) != SOCKET_ERROR){
                    printf("Connecté, bienvenue %s !\n",boeuf);
                }
            }
        else{
            dalmatien.message = loginKO;
            send(sock,&dalmatien,paquet_size,0) != SOCKET_ERROR;
            printf("Aucun compte avec ce username\n");
        }
      }
    }
...
int register_account(int sock,paquet* buffer){
   ...
   printf("username = %s ; password = %s\n",buffer->login,buffer->password);

    FILE * registre = NULL;

    registre = fopen("username.txt","a+");
